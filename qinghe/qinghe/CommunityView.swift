            ...
            HStack(spacing: 20) {
                ...
                ForEach(viewModel.channels) { channel in
                    Button(action: {
                        ...
                        Text(channel.name)
                            .font(.system(size: 16, weight: viewModel.selectedChannel?.id == channel.id ? .semibold : .regular))
                            .foregroundColor(viewModel.selectedChannel?.id == channel.id ? .primary : .secondary)
                            .fixedSize() // 确保文字不被压缩
                    }
                    .disabled(viewModel.isLoading)
                }
        ...
        ZStack {
            ...
            WaterfallLayout(
                ...
                WaterfallPostCard(
                    ...
                    onUserTap: {
                        selectedUserId = String(post.author.id)
                        showingUserProfile = true
                    }
                )
            }
