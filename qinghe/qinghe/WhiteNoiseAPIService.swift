...
// MARK: - 分页数据模型
struct WhiteNoisePaginatedResponse<T: Codable>: Codable {
    let list: [T]
    let pagination: WhiteNoisePagination
}

struct WhiteNoisePagination: Codable {
    let total: Int
    let page: Int
    let limit: Int
    let pages: Int
...
class WhiteNoiseAPIService {
    ...
    func getWhiteNoiseList(
        ...
        page: Int = 1,
        limit: Int = 20,
        completion: @escaping (Result<WhiteNoisePaginatedResponse<WhiteNoise>, Error>) -> Void
    ) {
