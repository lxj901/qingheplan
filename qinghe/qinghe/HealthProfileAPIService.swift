...
final class HealthProfileAPIService {
    ...
    /// 更新基础健康信息
    func updateHealthProfile(height: Double, weight: Double, bloodType: String?) async throws {
        ...
        let response: HealthAPIResponse<HealthProfile> = try await NetworkManager.shared.put(
            endpoint: "/health/profile/basic",
        ...
        guard response.success else {
            throw NSError(domain: "HealthProfileAPI", code: 2, userInfo: [
                NSLocalizedDescriptionKey: response.error ?? response.message ?? "更新健康信息失败"
            ])
        }
    }
