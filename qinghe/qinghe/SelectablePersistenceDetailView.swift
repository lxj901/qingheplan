...
/// 可选择的打卡历史页面 - 用于发布帖子时选择打卡记录
struct SelectablePersistenceDetailView: View {
    ...
    @State private var selectedRecord: CheckinHistoryRecord?
        ...
        HStack {
            ...
            Button(action: {
                if let selectedRecord = selectedRecord {
                    ...
                    let checkinData = convertToCheckinDataForPost(selectedRecord)
                    onSelection(checkinData)
                } else {
                    onSelection(nil)
                }
                dismiss()
                ...
                Text("确定")
                    ...
                    .foregroundColor(selectedRecord != nil ? .green : .gray)
            }
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 16) {
                ...
                VStack(spacing: 8) {
                    ...
                    Circle()
                        .fill(
                            LinearGradient(
                                colors: selectedRecord?.id == record.id ? [
                                    Color.blue,
                        ...
                        .shadow(
                            color: selectedRecord?.id == record.id ? Color.blue.opacity(0.3) :
                                   record.hasCheckin ? Color.green.opacity(0.3) : Color.gray.opacity(0.2),
                            radius: 2, x: 0, y: 1
                        )

                    // 连接线 - 根据是否选中显示不同颜色
                    Rectangle()
                        .fill(selectedRecord?.id == record.id ? Color.blue.opacity(0.3) :
                              record.hasCheckin ? Color.green.opacity(0.3) : Color.gray.opacity(0.2))
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    .fill(selectedRecord?.id == record.id ? Color.blue.opacity(0.05) : Color(.systemBackground))
                    .stroke(
                        selectedRecord?.id == record.id ? Color.blue.opacity(0.3) : Color.clear,
                        lineWidth: selectedRecord?.id == record.id ? 2 : 0
                    )
                    .shadow(
                        color: Color.black.opacity(0.05),
                        radius: 8,
    ...
    // MARK: - 转换方法
    private func convertToCheckinDataForPost(_ record: CheckinHistoryRecord) -> CheckinDataForPost? {
        guard record.hasCheckin, let checkinId = record.checkinId else { return nil }

        return CheckinDataForPost(
            checkinId: checkinId, // 使用真正的打卡记录ID
            date: record.date,
            location: nil, // 可以根据需要从record中获取位置信息
            note: record.note, // 使用record中的备注信息
            consecutiveDays: viewModel.totalConsecutiveDays, // 使用历史最长连续天数
...
// MARK: - 预览
struct SelectablePersistenceDetailView_Previews: PreviewProvider {
    static var previews: some View {
        SelectablePersistenceDetailView { checkinData in
