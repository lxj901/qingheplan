...
struct ChatUser: Codable, Identifiable, Equatable {
    ...
    // 标准初始化方法
    init(id: Int, nickname: String, avatar: String? = nil, isVerified: Bool? = nil, isOnline: Bool? = nil, lastSeenAt: String? = nil, isMember: Bool? = nil) {
        self.id = id
    ...
    init(from decoder: Decoder) throws {
        ...
        // 处理 isMember 字段：可能是数字或布尔值
        if let memberInt = try? container.decodeIfPresent(Int.self, forKey: .isMember) {
    ...
    static func fromDictionary(_ dict: [String: Any]) throws -> ChatUser {
        ...
        // 处理 isMember 字段：可能是数字或布尔值
        var isMember: Bool?
        if let memberInt = dict["isMember"] as? Int {
            isMember = memberInt != 0
        } else if let memberBool = dict["isMember"] as? Bool {
            isMember = memberBool
        }
