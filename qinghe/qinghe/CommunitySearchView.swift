...
struct UserSearchResultCard: View {
    ...
    private func loadFollowStatus() async {
        do {
            let response = try await CommunityAPIService.shared.getFollowStatus(userId: user.id)
            if response.success, let data = response.data {
                await MainActor.run {
                    isFollowing = data.isFollowing
                    print("🔍 CommunitySearchView: 关注状态更新为: \(data.isFollowing)")
                }
            } else {
                print("🔍 CommunitySearchView: 获取关注状态失败: \(response.message ?? "未知错误")")
            }
        } catch {
            print("🔍 CommunitySearchView: 获取关注状态异常: \(error.localizedDescription)")
        }
    }
}
