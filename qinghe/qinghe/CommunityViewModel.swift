...
class CommunityViewModel: ObservableObject {
    ...
    private var currentSwitchTask: Task<Void, Never>?  // 用于取消滑动切换任务
    ...
    // 按 tab 缓存帖子数据（改为 public，供视图直接访问）
    @Published var postsByTab: [CommunityTab: [Post]] = [:]
    // 按 tab 缓存分页信息
    private var pageByTab: [CommunityTab: Int] = [:]
    private var hasMorePostsByTab: [CommunityTab: Bool] = [:]
    ...
    func loadPosts(refresh: Bool = false, isLoadingMore: Bool = false) async {
        print("🔍 loadPosts 调用 - refresh: \(refresh), isLoadingMore: \(isLoadingMore), selectedTab: \(selectedTab.displayName)")
        ...
        if refresh {
            ...
            hasMorePosts = true
            posts = []  // 清空现有帖子
            isLoading = false  // 重置加载状态，允许新的刷新请求
        }

        // 检查是否可以加载（刷新时已经重置了 hasMorePosts 和 isLoading）
        ...
        // 创建新的任务，使用 defer 确保 isLoading 总是被重置
        currentLoadTask = Task { @MainActor in
            defer {
                isLoading = false
                print("🔍 [defer] 加载状态已重置: isLoading = false")
            }

            do {
                print("🔄 开始网络请求 - tab: \(selectedTab.displayName), page: \(currentPage)")
                ...
                if selectedTab == .nearby {
                    ...
                    if currentLatitude == nil || currentLongitude == nil {
                        ...
                        try await withThrowingTaskGroup(of: Void.self) { group in
                            ...
                            group.addTask {
                                ...
                                throw NSError(domain: "LocationTimeout", code: -1, userInfo: [NSLocalizedDescriptionKey: "获取位置超时"])
                            }
                    ...
                    print("📍 位置获取成功: (\(latitude), \(longitude))")
                ...
                // 保存到对应 tab 的缓存
                postsByTab[selectedTab] = posts
                pageByTab[selectedTab] = currentPage
                hasMorePostsByTab[selectedTab] = hasMorePosts

                // 标记当前 tab 已初次加载完成
                hasInitialLoaded[selectedTab] = true
                // 更新最后加载时间
                lastLoadTime[selectedTab] = Date()
                print("📦 缓存已更新，tab: \(selectedTab.displayName), 帖子数: \(posts.count)")
    ...
    func switchTab(_ tab: CommunityTab) async {
        ...
        // 无论是否是同一个标签，都强制刷新数据
        print("🔄 点击标签 \(tab.displayName)，强制刷新数据")
        await loadPosts(refresh: true, isLoadingMore: false)
    }

    // MARK: - 切换Tab（滑动切换时调用，同步版本，不刷新数据，使用缓存）
    func switchTabWithoutRefreshSync(_ tab: CommunityTab) {
        print("👆 滑动切换到标签 \(tab.displayName)")

        selectedTab = tab

        // 检查是否有缓存数据
        if let cachedPosts = postsByTab[tab] {
            // 从缓存恢复数据（同步操作，不卡顿）
            posts = cachedPosts
            currentPage = pageByTab[tab] ?? 1
            hasMorePosts = hasMorePostsByTab[tab] ?? true
            print("📦 从缓存恢复数据，帖子数: \(posts.count)")
        } else {
            // 没有缓存，异步加载数据
            print("📭 该标签没有缓存，开始异步加载数据")
    ...
    // MARK: - 切换Tab（滑动切换时调用，异步版本，保留用于兼容）
    func switchTabWithoutRefresh(_ tab: CommunityTab) async {
        // 取消之前的切换任务，避免竞争条件
        currentSwitchTask?.cancel()
    ...
    private func loadNearbyPosts(page: Int = 1) async throws -> PostListResponse {
        ...
        // 将 NearbyPostsResponse 转换为 PostListResponse
        guard let nearbyData = nearbyResponse.data else {
            throw APIError.invalidData("同城帖子数据为空")
        }

        // 将 NearbyPost 转换为 Post
        let posts = nearbyData.items.map { $0.toPost() }

        return PostListResponse(
            items: posts,
            pagination: nearbyData.pagination,
    ...
    func loadChannels() async {
        ...
        do {
            let channelList = try await communityService.getChannels()
            channels = channelList
            print("✅ 成功加载 \(channelList.count) 个频道")

            // 推荐标签不自动选中频道，显示所有频道的聚合内容
            // 只有在其他标签或手动选择时才设置 selectedChannel
            print("📺 推荐标签显示所有频道的聚合内容")
        } catch {
    ...
    /// 选择频道
    /// - Parameter channel: 要选择的频道，传nil表示选择全部
    func selectChannel(_ channel: Channel?) async {
        selectedChannel = channel
        print("📺 选择频道: \(channel?.name ?? "全部")")
