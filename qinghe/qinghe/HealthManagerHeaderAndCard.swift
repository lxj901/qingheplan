...
struct TongueDiagnosisView: View {
    ...
    let mode: Mode
    let conversationId: String?  // 从对话中跳转时传递
    @Environment(\.dismiss) private var dismiss
    ...
    init(mode: Mode = .tongue, conversationId: String? = nil) {
        self.mode = mode
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .principal) {
                Text(mode == .tongue ? "AI舌象检测" : "AI面部检测").font(.system(size: 17, weight: .semibold))
            }
        }
        .navigationBarTitleDisplayMode(.inline)
    ...
    private func analyzeImage(_ image: UIImage) async {
        ...
        do {
            ...
            // 如果是从对话中跳转过来的，发送诊断结果回对话并返回对话页面
            if let conversationId = conversationId {
                do {
                    print("📤 发送诊断结果到对话: \(conversationId)")
                    let diagnosisType = mode == .tongue ? "tongue" : "face"
                    let response = try await HealthChatAPIService.shared.sendDiagnosisResult(
                        conversationId: conversationId,
                        diagnosisType: diagnosisType,
                        imageUrl: imageUrl,
                        result: finalAnalysisData
                    )
                    print("✅ 诊断结果已发送到对话，准备返回对话页面")

                    // 直接通知对话页插入消息，减少等待用户刷新
                    if let data = response.data {
                        NotificationCenter.default.post(name: .healthDiagnosisNewMessage, object: data)
                    }
                    ...
                    return // 直接返回，不跳转到报告页面

                } catch {
                    print("⚠️ 发送诊断结果到对话失败: \(error)")
                    // 如果发送失败，仍然跳转到报告页面
                }
            }

            // 如果不是从对话发起的，或者发送失败，跳转到报告页面
            await MainActor.run {
...
private struct HealthBasicInfoEditSheet: View {
    ...
    @Environment(\.dismiss) private var dismiss
    @StateObject private var healthDataManager = HealthProfileDataManager.shared
    private let apiService = HealthProfileAPIService.shared
            ...
            Form {
                Section("体质信息") {
                    HStack {
                        Text("体质")
                        Spacer()
                        Text(constitutionValue)
                            .foregroundStyle(.secondary)
                    }
                    Text("体质由舌诊、面诊等分析得出，不可手动编辑")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                Section("其他") {
                    HStack {
                        Text("血型")
                        Spacer()
                        TextField("请输入血型", text: $bloodType)
                            .multilineTextAlignment(.trailing)
                    }
                }
            }
            .navigationTitle("编辑健康数据")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
    ...
    private var constitutionValue: String {
        let constitution = healthDataManager.primaryConstitution
        return constitution.isEmpty || constitution == "--" ? "未知" : constitution
    }
    ...
    private func save() async {
        ...
        // 调用 API 保存到服务器
        if let heightValue = info.heightCM,
           let weightValue = info.weightKG {
            do {
                try await apiService.updateHealthProfile(
                    height: heightValue,
                    weight: weightValue,
...
/// 我的睡眠卡片
private struct MySleepCard: View {
    @ObservedObject private var sleepManager = SleepDataManager.shared

    var body: some View {
        NavigationLink(destination: SleepRecordsView()) {
            VStack(alignment: .leading, spacing: 16) {
                Text("我的睡眠")
                    .font(.system(size: 20, weight: .semibold))
                    .foregroundStyle(Color(hex: "1F2A60"))
    ...
    private var formattedYesterdaySleep: String {
        // 获取昨天的睡眠记录
        ...
        // 筛选昨天的睡眠记录
        let yesterdayRecords = sleepManager.sleepRecords.filter { record in
            let recordStart = record.bedTime
            let recordEnd = record.wakeTime
            return (recordStart < endOfYesterday) && (recordEnd >= startOfYesterday)
        }

        guard !yesterdayRecords.isEmpty else { return "--" }

        let totalDuration = yesterdayRecords.reduce(0) { $0 + $1.totalSleepDuration }
...
/// 运动数据卡片 - 简化版，只显示步数和热量
private struct ExerciseDataCard: View {
    @StateObject private var healthDataManager = HealthProfileDataManager.shared
    @State private var showingWorkoutAnalysis = false
    ...
    private func loadTodayData() {
        HealthKitManager.shared.queryTodayStepsAndCalories { steps, calories in
            self.todaySteps = steps
            self.todayCalories = calories
            self.isLoadingData = false
            print("📊 今日运动数据 - 步数: \(steps), 热量: \(String(format: "%.0f", calories))kcal")
        }
    }
    ...
    private var stepsValue: String {
        ...
        return todaySteps > 0 ? "\(todaySteps)" : "--"
    }
    ...
    private var caloriesValue: String {
        ...
        return todayCalories > 0 ? String(format: "%.0f", todayCalories) : "--"
    }
...
/// 运动指标项（小卡片）
private struct ExerciseMetricItem: View {
    let icon: String
    let title: String
    let value: String
    let subtitle: String
...
private struct HealthDataCard: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    .foregroundStyle(Color(hex: "6B63FF"))
                    .padding(.horizontal, 12)
                    .padding(.vertical, 6)
                    .background(
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .fill(Color(hex: "6B63FF").opacity(0.1))
                    )
                }
            }
    ...
    private func loadCurrentData() {
        // 从 healthDataManager 加载数据到 basicInfo
        if let profile = healthDataManager.healthProfile {
            basicInfo.heightCM = profile.height
            basicInfo.weightKG = profile.weight
            basicInfo.bloodType = profile.bloodType
            basicInfo.age = profile.healthMetrics?.age
        }
