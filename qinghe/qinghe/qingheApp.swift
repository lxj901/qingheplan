...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        // 初始化阿里云人脸核身SDK
        FaceVerificationManager.initializeSDK()
...
struct qingheApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                ...
                // 悬浮球音频控制（全局显示）
                if !showSplash && authManager.isAuthenticated {
            ...
            .onAppear {
                ...
                // ✅ 不在启动时请求定位权限，延迟到用户需要时再请求
                // 定位权限将在用户点击社区同城标签或其他需要定位的功能时请求

                // 初始化推送通知（仅检查状态，不请求）
                Task {
                    await pushNotificationManager.checkNotificationPermission()

                    // 如果用户已认证，连接 WebSocket 并更新角标
            ...
            .onReceive(NotificationCenter.default.publisher(for: .navigateToActivities)) { _ in
                // 处理推送通知点击跳转到活动页面
    ...
    /// 处理导航到互动消息页面
    private func handleNavigateToNotifications() {
        print("🔔 导航到互动消息页面")
        // 切换到消息Tab，然后导航到NotificationListView
        NavigationManager.shared.selectedTab = .messages
    }

    /// 处理导航到活动页面
    private func handleNavigateToActivities() {
        print("🔔 导航到活动页面")
        // 切换到个人中心Tab，然后导航到ActivityCenterView
        NavigationManager.shared.selectedTab = .profile
        // 延迟发送导航通知，确保Tab切换完成
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            NotificationCenter.default.post(
                name: NSNotification.Name("NavigateToActivityCenter"),
                object: nil
            )
        }
    }
