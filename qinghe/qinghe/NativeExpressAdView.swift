...
// MARK: - 瀑布流广告卡片视图（社区列表使用）
struct WaterfallAdCard: View {
    let adView: UIView
    let onAdClosed: (() -> Void)?

    @State private var adHeight: CGFloat = 85
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // 广告内容区域 - 使用动态高度
            DynamicHeightAdView(adView: adView, height: $adHeight)
...
// MARK: - 动态高度广告视图
struct DynamicHeightAdView: UIViewRepresentable {
    let adView: UIView
    ...
    private func updateHeight() {
        let adHeight = adView.bounds.height

        if adHeight > 0 {
            // 限制最小和最大高度
            let minHeight: CGFloat = 80
            let maxHeight: CGFloat = 400
            let finalHeight = min(max(adHeight, minHeight), maxHeight)

            if finalHeight != height {
                print("🎯 更新广告高度: \(adHeight) -> \(finalHeight)")
...
extension GDTAdManager {
    ...
    /// 获取适合详情页的广告尺寸（三小图双文样式）
    static func getDetailPageAdSize() -> CGSize {
        let screenWidth = UIScreen.main.bounds.width
        let adWidth = screenWidth - 32 // 左右各16的边距
        let adHeight: CGFloat = 85 // 三小图双文样式高度（推荐85-90）
        return CGSize(width: adWidth, height: adHeight)
    }
