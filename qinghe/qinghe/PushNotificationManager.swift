...
class PushNotificationManager: NSObject, ObservableObject {
    ...
    func didReceiveRemoteNotification(_ userInfo: [AnyHashable: Any]) async -> UIBackgroundFetchResult {
        ...
        case .like, .comment, .bookmark, .follow:
            await handleSystemNotification(notificationData)
        case .activity:
            await handleSystemNotification(notificationData)
        }
...
extension PushNotificationManager: @preconcurrency UNUserNotificationCenterDelegate {
    ...
    func handleNotificationTap(_ userInfo: [AnyHashable: Any]) async {
        ...
        case .like, .comment, .bookmark, .follow:
            // 跳转到互动消息页面（NotificationListView）
            await navigateToInteractionNotifications()
        case .activity:
            // 跳转到活动页面
    ...
    /// 导航到互动消息页面
    private func navigateToInteractionNotifications() async {
        // 发送导航通知
        NotificationCenter.default.post(
    ...
    /// 导航到活动页面
    private func navigateToActivities() async {
        // 发送导航通知
        NotificationCenter.default.post(
            name: .navigateToActivities,
            object: nil
        )
    }
}
...
extension Notification.Name {
    ...
    static let navigateToActivities = Notification.Name("NavigateToActivities")
}
