...
class CommunityAPIService {
    ...
    private enum Endpoint {
        ...
        static let channels = "/community/channels"  // 频道列表
    }
    ...
    func getPosts(
        ...
        if let channelKey = channelKey {
            parameters["channelKey"] = channelKey
        }
...
struct UserProfile: Codable, Identifiable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let boolValue = try? container.decode(Bool.self, forKey: .isMember) {
            isMember = boolValue
        } else if let intValue = try? container.decode(Int.self, forKey: .isMember) {
            isMember = intValue != 0
        } else {
            isMember = nil
        }
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case isFollowing, isFollowedBy, isBlocked, isMe, hasPassword, isMember
    }
}

// MARK: - 频道功能扩展
extension CommunityAPIService {
    ...
    func getChannels() async throws -> [Channel] {
        ...
        let response: ChannelsResponse = try await networkManager.get(
        ...
        guard response.success else {
            throw NetworkManager.NetworkError.networkError(response.message ?? "获取频道列表失败")
        }
